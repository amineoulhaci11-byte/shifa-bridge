...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ appointments, user, onUpdateStatus, onUpdateSettings }) => {
  ...
  const stats = useMemo(() => {
    // تم تعديل التصفية لتبحث عن CONFIRMED بدلاً من ACCEPTED
    const pending = appointments.filter(a => a.status === AppointmentStatus.PENDING);
    const todays = appointments.filter(a => a.status === AppointmentStatus.CONFIRMED && a.date === todayStr)
                               .sort((a, b) => parseInt(a.time) - parseInt(b.time));
    const completed = appointments.filter(a => a.status === AppointmentStatus.COMPLETED).length;
    const allAccepted = appointments.filter(a => a.status === AppointmentStatus.CONFIRMED).length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2rem] border border-amber-100 p-6 shadow-sm">
            ...
            {stats.pending.length === 0 ? <div className="p-8 text-center text-slate-400">لا توجد طلبات معلقة حالياً.</div> : <div className="space-y-4">{stats.pending.map(appt => <div key={appt.id} className="p-5 border border-slate-100 rounded-2xl bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div className="flex items-center gap-4 text-right"><div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center text-amber-500"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><div><p className="font-bold text-slate-800">{appt.patientName}</p><p className="text-xs text-slate-500 font-medium">{appt.date === todayStr ? 'اليوم' : appt.date} • الدور: {appt.time}</p></div></div><div className="flex items-center gap-2"><button onClick={() => onUpdateStatus(appt.id, AppointmentStatus.REJECTED)} className="flex-1 sm:flex-none px-4 py-2 border border-red-200 text-red-600 rounded-xl hover:bg-red-50 font-bold transition-colors text-sm">رفض</button><button onClick={() => onUpdateStatus(appt.id, AppointmentStatus.CONFIRMED)} className="flex-1 sm:flex-none px-6 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-bold transition-colors shadow-lg shadow-blue-100 text-sm">قبول الموعد</button></div></div>)}</div>}
          </div>
        </section>

        <section className="space-y-6">
           <div className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm text-right"><h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">إعدادات العيادة</h3><div className="space-y-5"><div><label className="text-sm font-bold text-slate-600 block mb-2">الحد الأقصى للمواعيد اليومية</label><input type="number" min="1" max="100" value={maxAppointments} onChange={(e) => setMaxAppointments(Number(e.target.value))} className="w-full text-center font-bold text-lg p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" /></div><div className="border-t border-slate-100 pt-4"><div className="flex items-center justify-between mb-2"><label className="text-sm font-bold text-slate-600">الرد التلقائي</label></div>{autoReplyEnabled && <div className="mt-3"><textarea value={autoReplyMessage} onChange={(e) => setAutoReplyMessage(e.target.value)} className="w-full text-sm p-3 bg-slate-50 border border-slate-200 rounded-xl h-20 resize-none" placeholder="اكتب الرسالة التلقائية هنا..." /></div>}</div><button onClick={handleSaveSettings} disabled={isSavingSettings} className="w-full py-3 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-100">{isSavingSettings ? 'جاري الحفظ...' : 'حفظ التعديلات'}</button>{saveMessage && <p className={`text-xs text-center font-medium p-2 rounded-lg mt-2 ${saveMessage.type === 'success' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>{saveMessage.text}</p>}</div></div>
        </section>
      </div>
    </div>
  );
